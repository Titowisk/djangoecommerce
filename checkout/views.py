...
class CartItemView(TemplateView):
    ...
    def get_formset(self, clear=False):
        # Gera o conjunto de formúlarios baseado no modelo CartItem, com o campo quantity.
        # O campo de preço, id e nome do produto já tá lá no HTML.
        CartItemFormSet = modelformset_factory(CartItem, fields=('quantity',), can_delete=True, extra=0)
        """
        Se houver a sessão, o conjunto de formulários(formset) serão criados com o 
        modelo de carrinho de compras(CartItem) que está na sessão.
        """
        ...
        if session_key:
            if clear:
                ...
                # A parte de data eu não entendi ????????????????????????
                formset = CartItemFormSet(
                    queryset=CartItem.objects.filter(cart_key=session_key),
                    data=self.request.POST or None
                )
        # Caso contrário não haverá carrinho de compras pois o cliente ainda não adicionou nenhum item.
        else:
