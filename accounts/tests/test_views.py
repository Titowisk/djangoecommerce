...
class RegisterViewTestCase(TestCase):
    ...
    def test_register_ok(self):
        data = {
            'username': 'robson', 'password1': 'teste123', 'password2': 'teste123',
            'email': 'test@test.com'
        }
    ...
    def test_register_error(self):
        data = {'username': 'robson', 'password1': 'teste123', 'password2': 'teste123'}
        response = self.client.post(self.register_url, data)
...
class UpdateUserViewTestCase(TestCase):
    ...
    def test_update_user_ok(self):
        ...
        # O usuario acessa a pagina update_user.html.
        response = self.client.get(self.url)
        # Como não está logado ele é redirecionado para a pagina de login.
        self.assertEquals(response.status_code, 302)
        # Na pagina de login ele deve entrar com username e password.
        self.client.login(username=self.user.username, password='123')
        response = self.client.post(self.url, data)
        # Após isso é novamente redirecionado para a página de Contas.
        accounts_index_url = reverse('accounts:index')
        self.assertRedirects(response, accounts_index_url)  # Erro aqui. 06:26 do video.
